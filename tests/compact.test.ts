...
test('fabric bucket has a fuller crown and asymmetric downturned hem, not a regular trapezoid', () => {
  ...
  const widths = [3, 4, 5, 6].map(y => row(y).filter(Boolean).length)
  expect(widths[1]).toBeGreaterThan(widths[0]!)
  expect(widths[2]).toBe(widths[1]) // fabric side stays upright before the brim
  expect(widths[3]).toBeGreaterThan(widths[2]!)
  expect(row(7).map(Boolean)).not.toEqual(row(7).map(Boolean).reverse())
  expect(row(5)).toContain('#fff0cb')
  expect(pixel(frame, 2, 6)).toBeNull()
  expect(pixel(frame, 15, 6)).toBeNull()
  expect(pixel(frame, 2, 7)).toBeTruthy()
  expect(pixel(frame, 15, 7)).toBeTruthy()
  expect(pixel(frame, 8, 6)).toBeTruthy()
  expect(pixel(frame, 8, 7)).toBeNull()
})

test('neutral and thinking eyes are two-by-two squares through the glance, with hat optional', () => {
  for (const mood of ['idle', 'thinking'] as const) for (const hat of ['none', 'lavender-bucket']) {
    for (const ms of [500, 1100, 2900, 3200]) {
      ...
      for (const x of [5 + dx, 11 + dx]) {
        ...
        expect(pixel(frame, x + 1, 10)).toBe('#193f62')
        expect(pixel(frame, x, 11)).toBe('#193f62')
        expect(pixel(frame, x + 1, 11)).toBe('#193f62')
        for (const [nx, ny] of [[x - 1, 10], [x - 1, 11], [x + 2, 10], [x + 2, 11], [x, 9], [x + 1, 9], [x, 12], [x + 1, 12]]) {
...
test('smaller calm eyes retain distinct horizontal blink, working brow and happy finished eyes', () => {
  for (const hat of ['none', 'lavender-bucket']) {
    const blink = compose(catalog, { ...appearance, hat }, 'idle', 0, true)
    const working = compose(catalog, { ...appearance, hat }, 'working', 0, false)
    const done = compose(catalog, { ...appearance, hat }, 'done', 0, false)
    for (const x of [5, 11]) {
      expect(pixel(blink, x, 10)).not.toBe('#193f62')
      expect(pixel(blink, x + 1, 11)).toBe('#193f62')
      expect(pixel(working, x, 9)).toBe('#193f62')
      expect(pixel(working, x + 1, 10)).toBe('#193f62')
      expect(pixel(done, x + 1, 10)).toBe('#193f62')
      expect(pixel(done, x - 1, 11)).toBe('#193f62')
      expect(pixel(done, x, 11)).not.toBe('#193f62')
    }
  }
})
