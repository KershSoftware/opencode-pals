...
See [the native trial contract and smoke workflow](character-workflow.md#one-off-native-trials) for exact types, examples and automated capture.
...
See [character-workflow.md](character-workflow.md) for a working second-character/two-skin example, accessory rules, live editing, deterministic mood checks, and event feedback.
