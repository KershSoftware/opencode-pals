...
Historical Task 8 packaging: the package was private/unpublished, version `0.1.0`, with `exports["./tui"] = "./dist/index.js"`. Its runtime allowlist contained `dist/index.js`, README, character workflow, and this evidence file (plus mandatory package.json). Artwork was compiled into the entry, with no external runtime art files. At that stage, `scripts/verify-package.ts` packed/extracted the archive and checked the exact five-file inventory, entry byte identity, art metafile inputs, and the three external runtime imports. Generated sessions/keys, test fixtures, local config, dependencies, and metafile were excluded. This historical inventory predates the public MIT npm packaging and bundled Node CLI; see [current package contents and verification](setup.md#acceptance-and-package-contents) and [current installation instructions](setup.md#quickstart-install-globally).
